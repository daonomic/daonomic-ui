...
function getUpDownPosition(tip, target, state, direction, alignMode, props) {
  ...
  const arrowSpacing =
    typeof props.distance === 'number' ? props.distance : defaultArrowSpacing;
...
function getLeftRightPosition(tip, target, state, direction, alignMode, props) {
  ...
  if (tip && state.showTip) {
    ...
    const arrowBottom =
      targetRect.top + scrollTop + halfTargetHeight + arrowSize;
...
function getArrowStyles(target, tip, direction, state, props) {
  ...
  const scrollLeft = getScrollLeft();
  const arrowSpacing =
    typeof props.distance === 'number' ? props.distance : arrowSize;
  ...
  switch (direction) {
    case 'right':
      return {
        ...
        borderRight:
          props.background !== ''
            ? `${arrowSize}px solid ${props.background}`
            : '',
    ...
    case 'up':
      return {
        left:
          state.showTip && tip
            ? targetRect.left + scrollLeft + halfTargetWidth - arrowSize
            : '-10000000px',
    ...
    default:
      return {
        ...
        borderBottom:
          props.background !== '' ? `10px solid ${props.background}` : '',
        borderLeft: `${arrowSize}px solid transparent`,
        borderRight: `${arrowSize}px solid transparent`,
      };
  }
...
export default function positions(direction, tip, target, state, props) {
  ...
  if (tip && state.showTip) {
    const testArrowStyles = getArrowStyles(
      target,
      tip,
      trimmedDirection,
      state,
    ...
    realDirection = getDirection(
      ...
      tip,
      target,
      props.arrow ? arrowSize : 0,
      bodyPadding,
      props.arrow && testArrowStyles,
    );
  }
  ...
  if (realDirection === 'up' || realDirection === 'down') {
    ...
    tipPosition = getLeftRightPosition(
      tip,
      target,
      state,
      realDirection,
